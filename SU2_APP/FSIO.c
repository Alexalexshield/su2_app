...
DISK gDiskData = {0};   // Global structure containing device information.

DISK* getDiskInfo() { return &gDiskData; };
...
int FSInit(void)
{
    ...
    gBufferZeroed = FALSE;
	gLastFATSectorRead = 0xFFFF;        // Global variable indicating which FAT sector was read last
	gNeedFATWrite = FALSE;              // Global variable indicating that there is information that needs to be written to the FAT
	gBufferOwner = NULL;                // Global variable indicating which file is using the data buffer
	gLastDataSectorRead = 0xFFFFFFFF;   // Global variable indicating which data sector was read last
	gNeedDataWrite = FALSE;             // Global variable indicating that there is information that needs to be written to the data section
	nextClusterIsLast = FALSE;          // Global variable indicating that the entries in a directory align with a cluster boundary
...
BYTE LoadBootSector(DISK *dsk)
{
    ...
    else
    {
        ...
        else
        {
                ...
                dsk->bytespersector = BytesPerSec;		// added by rene
...
BYTE PopulateEntries(FILEOBJ fo, char *name , WORD *fHandle, BYTE mode)
{
   ...
   // Timing information for uncontrolled clock mode
    ...
    if (Write_File_Entry(fo,fHandle) != TRUE)
        error = CE_FILE_ENTRY_WRITE_ERROR;
...
BOOL isFilePointerValid(FSFILE * p) 
{
	int fIndex;
	...
	for( fIndex = 0; fIndex < FS_MAX_FILES_OPEN; fIndex++ )
    {
        if((p == &gFileArray[fIndex]) && (gFileSlotOpen[fIndex] == FALSE))   //is this a valid pointer to an initialized structure
        {
	        return TRUE;
        }
    }
...
BYTE FormatFileName( const char* fileName, char* fN2, BYTE mode)
{
    ...
    if( temp <= TOTAL_FILE_SIZE ) // 8+3+1
        strncpy( szName, fileName, sizeof(szName) );  // copy to RAM in case fileName is located in flash
    else
