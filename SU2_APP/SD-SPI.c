...
#define R1_RESPONSE_TIMEOUT (16ul * PROCESSOR_MIPS)
#define R1_COMMAND_TIMEOUT ((unsigned long)PROCESSOR_MIPS * 0x8000ul)
...
void MDD_SDSPI_InitIO (void)
{
	...
	SD_WRITE_LED(0);				// ACTIVITY cleared
//   	SD_ACT_TRIS = OUTPUT;			// Make the SD activity pin an output
}
...
//        if (timeout == 0)
//        	traceS2("SendMMCCmd:","R1_RESPONSE_TIMEOUT");
    }
    ...
    if(sdmmc_cmdtable[cmd].responsetype == R1b)
    {
        ...
        for(index =0; index < 0xFF && response.r1._byte == 0x00; index++)
        {
	        ...
	        if (timeout == 0)
	        	traceS2("SendMMCCmd:","R1_COMMAND_TIMEOUT");
        }
    }
...
BYTE MDD_SDSPI_SectorRead(DWORD sector_addr, BYTE* buffer)
{
    ...
    if(response.r1._byte != 0x00)
    {
        ...
        {
            return FALSE;
	        traceS2("MDD_SDSPI_SectorRead:","timeout waiting for READ_SINGLE_BLOCK command response");
        }
    }
    ...
    {
        status = FALSE;
        traceS2("MDD_SDSPI_SectorRead:","timeout waiting for datatoken");
    }
    else
    {
#ifdef USB_USE_MSD
...
//    char msgbuf[30];
    ...
    else
    {
        ...
        else
        {
            ...
            {
                unsigned long timeout = 0x10000ul * PROCESSOR_MIPS;            
...
#endif
                    timeout--;
                }while((data_response == 0x00) && (timeout != 0));

                if(timeout == 0)   {                               //if timeout first
                    status = FALSE;
	                traceS2("MDD_SDSPI_SectorWrite:","timeout waiting for data response after write");
                }    
            }
...
/*
    ...
    return;
    */
    DelayMsecs((unsigned int)milliseconds);	// call routine that changes delay based on processor mips
...
MMC_RESPONSE    g_SDresponse; 

BYTE MDD_SDSPI_MediaInitialize(void)
{
    WORD timeout;
    BYTE       status = TRUE;
    g_SDresponse.r2._word = 0;
#if defined __C30__ || defined __C32__
    ...
    g_SDresponse = SendMMCCmd(GO_IDLE_STATE,0x0);
    
    if((g_SDresponse.r1._byte == MMC_BAD_RESPONSE) || ((g_SDresponse.r1._byte & 0xF7) != 0x01))
    {
        status = FALSE;      // we have not got anything back from the card
        SD_CS_DESELECT(); 	// deselect the devices
        if (g_SDresponse.r1._byte == MMC_BAD_RESPONSE) {
	        traceS2("MDD_SDSPI_MediaInitialize:","GO_IDLE_STATE response is MMC_FLOATING_BUS");
	    } 
        else {
	        traceS2("MDD_SDSPI_MediaInitialize:","GO_IDLE_STATE response error");
    	    traceR1Errors();                               
    	} 
        return status;
    }
    ...
    unsigned long tmout = 0x400ul * PROCESSOR_MIPS; 		// was 0xFFF;
    
    do
    {
        g_SDresponse = SendMMCCmd(SEND_OP_COND,0x0);
        tmout--;
    }while(g_SDresponse.r1._byte != 0x00 && tmout != 0);
    ...
    if(tmout == 0)
    {
        status = FALSE;      // we have not got anything back from the card
        SD_CS_DESELECT();                               // deselect the devices
        traceS2("MDD_SDSPI_MediaInitialize:","timeout waiting for SEND_OP_COND response");
    }
    else      
    {
...
//    Delayms(1); // removed by rene
    ...
    Delayms(1); 
        ...
        // Send CMD0 to reset the media
        g_SDresponse = SendMMCCmdManual (GO_IDLE_STATE, 0x0);
    	...
    	timeout = 0x400 * PROCESSOR_MIPS; 		// was 0xFFF;
    
        do
        {
            g_SDresponse = SendMMCCmdManual (SEND_OP_COND, 0x0);
            timeout--;
        }while(response.r1._byte != 0x00 && timeout != 0);
    #endif    
    ...
    else
    {
        ...
        g_SDresponse = SendMMCCmd(CRC_ON_OFF,0x0);

 		g_SDresponse.r2._word = 0;
        // Now set the block length to media sector size. It should be already
        g_SDresponse = SendMMCCmd(SET_BLOCKLEN,MEDIA_SECTOR_SIZE);
        
        timeout = 0x40 * PROCESSOR_MIPS; 		// was 0xFF;
        while ((timeout > 0) && (MDD_SDSPI_SectorRead(0x0,NULL) != TRUE)) {
        	timeout--;
        }	
...
//        {;}
        ...
        if(timeout == 0)
        {
            status = FALSE;
            SD_CS_DESELECT();                               // deselect the devices
       	 	traceS2("MDD_SDSPI_MediaInitialize:","timeout waiting for SET_BLOCKLEN response");
        }
    }
