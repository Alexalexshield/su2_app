...
//	asm("mov.b	W0, [W1]");		
	...
	asm volatile ("bset OSCCON, #1");	// 	_SOSCEN = 1;		// ENABLE SECONDARY OSCILLATOR
...
void RTCCEnableSecondAlarm(void)
{
	_ALRMEN=0;
	_ALRMPTR=0;			// SET POINTER TO WRITE TO THE MIN/SEC counter
	ALRMVAL=0x0000;	// min.sec = 0:0
	_ARPT =0;			// Alarm Repeat Counter is not important with chime enabled
	_CHIME = 1;			// indefinitely
	_AMASK = 0b0001;	// alarm every second
	_ALRMEN=1;			// enable alarm
	_RTCIF = 0;			// CLEAR THE FLAG
	_RTCIE = 1;			// enable the interrupt
...
*/
time_t RTCCGetTime_t()
{
	time_t time_t_time;
	struct tm updatetm;
	RTCCProcessEvents();
	updatetm.tm_year = mRTCCGetBinYear() + 2000 - 1900;
   	updatetm.tm_mon = mRTCCGetBinMonth()-1;
   	updatetm.tm_mday = mRTCCGetBinDay();
   	updatetm.tm_hour = mRTCCGetBinHour();
   	updatetm.tm_min = mRTCCGetBinMin();
   	updatetm.tm_sec = mRTCCGetBinSec();
	updatetm.tm_isdst = 0;
   	time_t_time = mktime(&updatetm);
   	return time_t_time;
}	
...
// todo - add range checking on values
void RTCCSetTime(RTCC *p)
{
	_time_chk = *p;
	RTCCSet();
}
